...
for frame1 in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if (int(classes[0][0]) == 1 and scores[0][0] > 0.8):
        ...
        area = (boxes[0][0][3] * IM_WIDTH) * (boxes[0][0][2]*IM_HEIGHT)
        ...
        area = (boxes[0][1][3] * IM_WIDTH) * (boxes[0][1][2] * IM_HEIGHT)
        ...
        area = (boxes[0][2][3] * IM_WIDTH) * (boxes[0][2][2] * IM_HEIGHT)
    ...
    if xcenter == 0 and ycenter == 0 and spin == 0:
        if i == 3:  # se non trovo nulla per almeno 2 ( 4 frame a 2 FPS )
            # LED BLUE ON
            print("Stop pwm and Spin")
            GP.output(LEDG, GP.LOW)
            if turn == 0:
                turn = 1
                pwm_default = 100  # guizzo iniziale per vincere attrito statico
                go_right(pwm_default)
                time.sleep(0.05)
                pwm_default = 60
                go_right(pwm_default)
            elif turn == 1:
        ...
        print("Stop spinning")
        spin = 0
    # q se ho trovato la persona solo che poiche si aggiorna ogni 0.5 sec circa allora lui continua a rotare, allora se il frame dopo
    # la persona trovata risulta una persona non  trovata allora deve ruotare nella direzione opposta  per mezzo secondo o poco più
    # poiche bisogna contare il fatto che si è fermata la macchinina e partenza da ferma ruota meno per l'attrito (a parita di tempo)
